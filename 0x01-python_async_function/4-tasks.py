...
async def task_wait_n(n: int, max_delay: int) -> List[float]:
    ...
    """
    wait_tasks = [task_wait_random(max_delay) for _ in range(n)]  # Create the tasks
    return [await wait_task for wait_task in asyncio.as_completed(wait_tasks)]  # Await the completion of tasks and return the results
